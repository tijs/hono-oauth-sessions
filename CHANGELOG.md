...
- **Dependencies**: Updated `@atproto/syntax` to 0.4.1 (from 0.4.0) to fix GitHub Actions publishing issue
...
1. **Profile Data**: Fetch profile data separately in your application code
   ...
   if (result.valid) {
     ...
     const profile = await oauthSession.makeRequest(
       "GET",
...
- **Enhanced Documentation**:
  - Added comprehensive examples for implementing custom OAuth clients with token refresh
...
- **Type Safety**:
  ...
  - Eliminated fragile workaround that violated Liskov Substitution Principle
...
- **Code Quality**: Improved architectural design and eliminated code smells
